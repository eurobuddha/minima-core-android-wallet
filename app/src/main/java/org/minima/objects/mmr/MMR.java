...
// M0 STUB: org.minima.database.MinimaDB import removed — the node DB is not bundled in the wallet.
// The only usage (getMegaMMR() chain-fallback in getEntry) is stubbed out below; it is not on the
// address/proof code path used by this wallet (mUseMegaMMR is false unless GeneralParams.IS_MEGAMMR).
		...
		while(current != null || MEGACHECK) {
			...
			if(!MEGACHECK) {
				...
				if(current == null && mUseMegaMMR) {
					...
					MEGACHECK = true;
					// M0 STUB: the node's MegaMMR is not available in the wallet build.
					// Original: current = MinimaDB.getDB().getMegaMMR().getMMR();
					// The wallet never enables IS_MEGAMMR, so this branch is unreachable in practice;
					// break out safely rather than dereference the absent node DB.
					break;
				}
			}else {
